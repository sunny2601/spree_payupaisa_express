...
describe "Payupaisa", :js => true do
  let!(:product) { FactoryGirl.create(:product, :name => 'iPad') }
  before do
    @gateway = Spree::Gateway::PayupaisaExpress.create!({
  ...
  def fill_in_billing
    within("#billing") do
      ...
      # City, State and ZIP must all match for Payupaisa to be happy
  ...
  it "pays for an order successfully" do
    ...
    click_button "Save and Continue"
    find("#payupaisa_button").click
    switch_to_payupaisa_login
  ...
  it "includes adjustments in Payupaisa summary" do
    visit spree.root_path
    click_link 'iPad'
    click_button 'Add To Cart'
  ...
  context "cannot process a payment with invalid gateway details" do
    ...
    specify do
      ...
      click_button "Save and Continue"
      find("#payupaisa_button").click
      page.should have_content("Payupaisa failed. Security header is not valid")
    end
  end
  ...
  context "as an admin" do
    ...
    context "refunding payments" do
      before do
        ...
        click_link "Payments"
        click_link "Payupaisa"
        click_link "Refund"
      end

      it "can refund payments fully" do
        click_button "Refund"
        page.should have_content("Payupaisa refund successful")
      ...
      it "errors when given an invalid refund amount" do
        ...
        click_button "Refund"
        page.should have_content("Payupaisa refund unsuccessful (The partial refund amount is not valid)")
      end
    end
  end
end
